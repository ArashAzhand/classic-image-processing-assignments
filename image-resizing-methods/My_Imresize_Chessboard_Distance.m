function Output_Image = My_Imresize_Chessboard_Distance(Input_Image, Resizing_Factor)
    ...
    for i=1:new_x
        for j=1:new_y
            for k=1:num_channels
                ...
                % chessboard
                top_left_dis = max(abs(x1 - i), abs(y1 - j));
                top_right_dis = max(abs(x2 - i), abs(y1 - j));
                down_left_dis = max(abs(x1 - i), abs(y2 - j));
                down_right_dis = max(abs(x2 - i), abs(y2 - j));
